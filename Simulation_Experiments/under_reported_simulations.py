...
if complex_graph:
    ...
    lambda_1, lambda_2, _ = best_lambdas_simple.loc[m][(low_var, False)]
...
for tt in range(rounds):
    ...
    trial = 10
    #y = np.mean([np.random.binomial(n.astype(int), p) for _ in range(trial)], axis=0)
    y = np.mean([n - np.random.poisson(n * p) for _ in range(trial)], axis=0)
    H = np.eye(m) - X @ np.linalg.pinv(X.T @ X) @ X.T

    # Initializations
    n_est = y
    # v_est represents 1-p
    v_est = np.zeros(m)+1.0 

    step = 0.4
    inner_iter = 1

    for ii in range(outer_iter):
        # We calculate the relative relative L1 error on the whole graph
        ...
        # training loss
        loss[ii] = np.linalg.norm(y - n_est * (1-v_est)) ** 2 + lambda_1 * (np.log(v_est)).T @ L @ (np.log(v_est)) + lambda_2 * np.log(n_est).T @ H @ np.log(n_est)
...
if low_var:
    if large_n:
        results_dir = "Simulation_Experiments/new_under_reported/Low_Var_p/Large_n"
    else:
        results_dir = "Simulation_Experiments/new_under_reported/Low_Var_p/Small_n"
else:
    if large_n: 
        results_dir = "Simulation_Experiments/new_under_reported/High_Var_p/Large_n"
    else:
        results_dir = "Simulation_Experiments/new_under_reported/High_Var_p/Small_n"
...
if rounds > 1:
    ...
    plt.vlines(x1, 0, 1-v_est, colors=colors[1], label=r'Estimated $p_{\mathrm{est}}$',linewidth=0.8)
    plt.scatter(x1, 1-v_est, color=colors[1], zorder=3, s=20)
    ...
    # x2 = np.arange(1, m + 1) + 0.25  # Shift right for true values
    # x3 = np.arange(1, m + 1) - 0.25  # Shift left for observed y
    # plt.vlines(x2, 0, n / 1e3, colors='b', label='True n',linewidth=0.8)
    # plt.vlines(x1, 0, n_est / 1e3, colors='r', label='Estimated n',linewidth=0.8)
    # plt.vlines(x3, 0, y / 1e3, colors='g', label='Observed y',linewidth=0.8)
    # plt.scatter(x1, n_est / 1e3, color='r', zorder=3, s=20)
    # plt.scatter(x2, n / 1e3, color='b', zorder=3, s=20)
    # plt.scatter(x3, y / 1e3, color='g', zorder=3, s=20)
    # plt.xlabel("Node index")
    ...
    # x1 = np.arange(1, m+1) - 0.10  # Shift left for estimated values
    # x2 = np.arange(1, m+1) + 0.10  # Shift right for true values
    # plt.vlines(x2, 0, p, colors='b', label='True p',linewidth=0.8)
    # plt.vlines(x1, 0, -v_est + 1, colors='r', label='Estimated p',linewidth=0.8)
    # plt.scatter(x1, -v_est + 1, color='r', zorder=3, s=20)
    # plt.scatter(x2, p, color='b', zorder=3, s=20)
    # plt.xlabel("Node index")
    # plt.ylabel("Value")
    # plt.title(r"Comparison of $\mathrm{p}$ and $\mathrm{p}_{\mathrm{est}}$")
    # plt.ylim(0, 0.8)  # Set y-axis limits from 0 to 0.8
    ...
    # # Relative L1 error of n
    # plt.figure()
    # plt.plot(relative_err_n, label=r"Relative Error of $\mathrm{n}_{\mathrm{est}}$", color="blue", linestyle="--")
    # plt.ylabel(r"Relative $\mathrm{L}_\mathrm{1}$ Error of $\mathrm{n}_{\mathrm{est}}$")
    # plt.ylim(0, 1)
    # plt.xlabel("Iteration")
    # plt.title(r"Convergence of Relative $\mathrm{n}_{\mathrm{est}}$ Error")
    # final_n = relative_err_n[-1]
    # plt.annotate(f'{final_n:.2f}', xy=(len(relative_err_n) - 1, final_n), 
    #             xytext=(len(relative_err_n) - 1.5, final_n + 0.05),
    #             arrowprops=dict(arrowstyle='-', color='blue'),
    #             fontsize=10, color="blue")
    # plt.legend(loc="upper right")
    # plt.tight_layout()
    # plt.savefig(results_dir+f"/n_error_Normalized_m={m}_K={K}_lambda_={lambda_1},{lambda_2}_iter={outer_iter}_inner={inner_iter}.png")
    # plt.savefig(results_dir+f"/n_error_Normalized_m={m}_K={K}_lambda_={lambda_1},{lambda_2}_iter={outer_iter}_inner={inner_iter}.pdf")

    # # Relative L1 error of p
    # plt.figure()
    # plt.plot(relative_err_p, label=r"Relative Error of $\mathrm{p}_{\mathrm{est}}$", color="red", linestyle="--")
    # plt.ylabel(r"Relative $\mathrm{L}_\mathrm{1}$ Error of $\mathrm{p}_{\mathrm{est}}$")
    # plt.ylim(0, 1)
    # plt.xlabel("Iteration")
    # plt.title(r"Convergence of Relative $\mathrm{p}_{\mathrm{est}}$ Error")
    # final_p = relative_err_p[-1]
    # plt.annotate(f'{final_p:.2f}', xy=(len(relative_err_p) - 1, final_p), 
    #             xytext=(len(relative_err_p) - 1.5, final_p + 0.05),
    #             arrowprops=dict(arrowstyle='-', color='red'),
    #             fontsize=10, color="red")
    # plt.legend(loc="upper right")
    # plt.tight_layout()
    # plt.savefig(results_dir+f"/p_error_Normalized_m={m}_K={K}_lambda_={lambda_1},{lambda_2}_iter={outer_iter}_inner={inner_iter}.png")
    # plt.savefig(results_dir+f"/p_error_Normalized_m={m}_K={K}_lambda_={lambda_1},{lambda_2}_iter={outer_iter}_inner={inner_iter}.pdf")

    # # L1 error of p
    # plt.figure()
    # plt.plot(l1_err_p, label=r"$\mathrm{L}_\mathrm{1}$ Error of $\mathrm{p}_{\mathrm{est}}$", color="red", linestyle="--")
    # plt.ylabel(r"$\mathrm{L}_\mathrm{1}$ Error of $\mathrm{p}_{\mathrm{est}}$")
    # plt.ylim(0, 1)
    # plt.xlabel("Iteration")
    # plt.title(r"Convergence of $\mathrm{L}_\mathrm{1}$ Error of $\mathrm{p}_{\mathrm{est}}$")
    # final_p = l1_err_p[-1]
    # plt.annotate(f'{final_p:.2f}', xy=(len(l1_err_p) - 1, final_p), 
    #             xytext=(len(l1_err_p) - 1.5, final_p + 0.05),
    #             arrowprops=dict(arrowstyle='-', color='red'),
    #             fontsize=10, color="red")
    # plt.legend(loc="upper right")
    # plt.tight_layout()
    # plt.savefig(results_dir+f"/p_error_L1_Normalized_m={m}_K={K}_lambda_={lambda_1},{lambda_2}_iter={outer_iter}_inner={inner_iter}.png")
    # plt.savefig(results_dir+f"/p_error_L1_Normalized_m={m}_K={K}_lambda_={lambda_1},{lambda_2}_iter={outer_iter}_inner={inner_iter}.pdf")

    # # Linf error of p
    # plt.figure()
    # plt.plot(linf_err_p, label=r"$\mathrm{L}_\mathrm{inf}$ Error of $\mathrm{p}_{\mathrm{est}}$", color="red", linestyle="--")
    # plt.ylabel(r"$\mathrm{L}_\mathrm{inf}$ Error of $\mathrm{p}_{\mathrm{est}}$")
    # plt.ylim(0, 1)
    # plt.xlabel("Iteration")
    # plt.title(r"Convergence of $\mathrm{L}_\mathrm{inf}$ Error of $\mathrm{p}_{\mathrm{est}}$")
    # final_p = linf_err_p[-1]
    # plt.annotate(f'{final_p:.2f}', xy=(len(linf_err_p) - 1, final_p), 
    #             xytext=(len(linf_err_p) - 1.5, final_p + 0.05),
    #             arrowprops=dict(arrowstyle='-', color='red'),
    #             fontsize=10, color="red")
    # plt.legend(loc="upper right")
    # plt.tight_layout()
    # plt.savefig(results_dir+f"/p_error_Linf_Normalized_m={m}_K={K}_lambda_={lambda_1},{lambda_2}_iter={outer_iter}_inner={inner_iter}.png")
    # plt.savefig(results_dir+f"/p_error_Linf_Normalized_m={m}_K={K}_lambda_={lambda_1},{lambda_2}_iter={outer_iter}_inner={inner_iter}.pdf")
